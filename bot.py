...
async def emoji_id_command(message: Message):
    ids = extract_custom_emoji_ids(message)
    if not ids:
        await message.answer(
            "❌ এই message-এ কোনো Custom Emoji entity পাওয়া যায়নি।\n"
            "Premium/Custom Emoji-সহ message-এর সাথে /emojiid ব্যবহার করুন।"
        )
        return
    await message.answer(
        "🆔 <b>Custom Emoji ID</b>\n\n" +
...
async def catch_all(message: Message):
    ...
    if step == "button_name":
        ...
        if not name:
            ...
            return
        # Do not print the emoji ID in the button text. Store it separately
        # and pass it to InlineKeyboardButton.icon_custom_emoji_id.
        state["button_name"] = name
        state["step"] = "button_emoji"
        await message.answer(
            "✨ <b>Premium/Custom Emoji ID</b> পাঠান।\n\n"
            "উদাহরণ: <code>6235336389647407554</code>\n"
            "Emoji না চাইলে <b>skip</b> লিখুন।"
        )
        return
    ...
    if step == "button_emoji":
        ...
        if emoji_id.lower() == "skip":
            ...
            await message.answer("❌ শুধু numeric Custom Emoji ID দিন, অথবা <b>skip</b> লিখুন।")
            return
