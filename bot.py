...
def init_db():
    ...
    """)
    # Migrate older databases created before media support.
    for col, definition in [("media_type", "TEXT DEFAULT ''"), ("media_file_id", "TEXT DEFAULT ''")]:
        try:
...
def save_draft(user_id, description, entities, media_type="", media_file_id=""):
    con = db()
    con.execute(
        """INSERT INTO drafts(user_id,description,description_entities,media_type,media_file_id)
           VALUES(?,?,?,?,?)
        ...
        (user_id, description, entities, media_type, media_file_id)
    )
    con.commit()
    con.close()
...
def parse_emoji_id(text):
    # Optional format:
    # Button name | emoji_id
    # Description can use Telegram custom emoji entities directly when pasted
    # from a Telegram client. For buttons, Telegram Bot API does not support
    # MessageEntity custom_emoji inside InlineKeyboardButton.text.
    if "|" not in text:
        return text.strip(), ""
...
def build_markup(rows):
    ...
    row = []
    for b in rows:
        # InlineKeyboardButton.text is plain text in Bot API; custom emoji
        # entities cannot be attached to button labels. The stored emoji_id is
        # preserved for future rendering/management.
        label = b["name"]
        row.append(InlineKeyboardButton(text=label, url=b["url"]))
        if len(row) == 2:
            keyboard.append(row)
            row = []
    if row:
...
async def send_draft(chat_id, user_id):
    ...
    try:
        ...
        entities = []
    caption = draft["description"] or ""
    common = {"chat_id": chat_id, "reply_markup": build_markup(buttons)}
    if draft["media_type"] == "photo" and draft["media_file_id"]:
        kwargs = {**common, "photo": draft["media_file_id"], "caption": caption}
        if entities:
            kwargs["caption_entities"] = entities
        await bot.send_photo(**kwargs)
    elif draft["media_type"] == "video" and draft["media_file_id"]:
        kwargs = {**common, "video": draft["media_file_id"], "caption": caption}
        if entities:
            kwargs["caption_entities"] = entities
        await bot.send_video(**kwargs)
    else:
        kwargs = {**common, "text": caption}
        if entities:
...
async def catch_all(message: Message):
    ...
    if step in ("description", "content"):
        # Text, photo or video are all valid content. For media, the caption
        # becomes the description/caption and Telegram's entities are kept.
        import json
        entities = []
        incoming_entities = message.caption_entities if (message.photo or message.video) else message.entities
        ...
        if message.photo:
            ...
            await message.answer("❌ Text, ছবি অথবা ভিডিও পাঠান।")
            return

        save_draft(uid, description, json.dumps(entities), media_type, media_file_id)
        state["step"] = "button_name"
        content_label = "ছবি" if media_type == "photo" else ("ভিডিও" if media_type == "video" else "Description")
        await message.answer(
    ...
    if step == "button_name":
        ...
        if not raw:
            ...
            return
        name, emoji_id = parse_emoji_id(raw)
        if not name:
            await message.answer("❌ Button Name দিন।")
            return
        state["button_name"] = name
        state["emoji_id"] = emoji_id
        state["step"] = "button_link"
        await message.answer("🔗 এখন Button-এর <b>Link</b> পাঠান।\nউদাহরণ: <code>https://example.com</code>")
        return
    ...
    if step == "button_link":
        ...
        if not valid_url(url):
            ...
            return
        button_name = state["button_name"]
        add_button(uid, button_name, url, state.get("emoji_id", ""))
        state.clear()
        state["step"] = "more"
        rows = get_buttons(uid)
        await message.answer(
            f"✅ Button added: <b>{button_name}</b>\n\n"
...
async def add_more(call: CallbackQuery):
    ...
    await call.message.edit_text(
        "🔘 <b>Button Name</b> দিন।\n\n"
        "Optional premium emoji ID format:\n"
        "<code>Button Name | 1234567890123456789</code>"
    )
