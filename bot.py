...
# Format supplied by the owner:
# emoji_id | button_name | emoji_id
PREMIUM_EMOJIS = {
    "admin_inbox": {
        "left": "6158862632926319619",
        "name": "𝐀𝐃𝐌𝐈𝐍 𝐈𝐍𝐁𝐎𝐗",
        "right": "6206112371308500200",
    },
}
...
async def pick_predefined_emoji(call: CallbackQuery):
    ...
    if key == "none":
        state["emoji_id"] = ""
    elif key in PREMIUM_EMOJIS:
        # The actual icon ID is stored separately from button text.
        # We use the right-hand ID for the displayed button icon.
        state["emoji_id"] = PREMIUM_EMOJIS[key]["right"]
    else:
