...
DEFAULT_ADMIN_INBOX = ("6158862632926319619", "𝐀𝐃𝐌𝐈𝐍 𝐈𝐍𝐁𝐎𝐗", "6206112371308500200")
...
def ensure_defaults():
    e = load_emojis()
    changed = False
    if "𝐀𝐃𝐌𝐈𝐍 𝐈𝐍𝐁𝐎𝐗" not in e:
        e["𝐀𝐃𝐌𝐈𝐍 𝐈𝐍𝐁𝐎𝐗"] = DEFAULT_ADMIN_INBOX[0]
        changed = True
    if changed:
        save_emojis(e)

def normalize_button_input(raw):
    """Return (label, emoji_id) and force the built-in ADMIN INBOX preset."""
    raw = raw.strip()
    # Exact requested preset: emoji_id | label | button_emoji_id
    parts = [x.strip() for x in raw.split("|")]
    if len(parts) >= 3 and parts[0] == DEFAULT_ADMIN_INBOX[0] and parts[2] == DEFAULT_ADMIN_INBOX[2]:
        return DEFAULT_ADMIN_INBOX[1], DEFAULT_ADMIN_INBOX[2]
    if raw.upper().replace(" ", "") == "ADMININBOX":
        return DEFAULT_ADMIN_INBOX[1], DEFAULT_ADMIN_INBOX[2]
    if len(parts) >= 2 and parts[1].strip().upper().replace(" ", "") == "𝐀𝐃𝐌𝐈𝐍𝐈𝐍𝐁𝐎𝐗":
        return DEFAULT_ADMIN_INBOX[1], DEFAULT_ADMIN_INBOX[2]
...
def button_markup(buttons, with_icons=True):
    if not buttons:
        return None
    rows=[]
    for item in buttons:
...
async def text_messages(update, context):
    ...
    if stage=="button_label":
        label, emoji_id = normalize_button_input(update.message.text)
        context.user_data["button_label"]=label
        context.user_data["button_emoji_id"]=emoji_id
...
async def publish(update,context):
    ...
    try:
        ...
        markup = button_markup(buttons, with_icons=True)
        try:
            ...
            # A custom-emoji icon can be rejected if the ID is not valid/eligible.
            # Retry with the same URL buttons but without the icon so the button itself
            # is never lost.
            if buttons:
                await context.bot.send_message(
                    chat_id=cid,
                    text=clean_text,
                    reply_markup=button_markup(buttons, with_icons=False)
                )
            else:
                raise first_error
        count=len(buttons)
